-- Base de datos AgriManage con Procedimientos Almacenados
...
-- Tabla de empresas/agroquímicos
...
-- 1. Procedimiento para crear empresa
...
-- 2. Procedimiento para crear usuario (ingeniero)
...
BEGIN
    ...
    -- Verificar si la cédula ya existe en la empresa
...
-- 4. Procedimiento para registrar lote
...
-- 5. Procedimiento para registrar etapa de cultivo
...
BEGIN
    ...
    -- Calcular días según tipo de cultivo
...
-- 6. Procedimiento para registrar aplicación de producto
...
BEGIN
    ...
    SELECT stock INTO v_stock_actual FROM productos WHERE id = p_id_producto AND deleted_at IS NULL;
    
    IF v_stock_actual < p_cantidad THEN
        SELECT 'Stock insuficiente' AS mensaje, 0 AS success;
        ROLLBACK;
    ELSE
...
-- 7. Procedimiento para finalizar etapa y registrar producción
...
BEGIN
    ...
    -- Calcular días reales de cultivo
...
-- 8. Procedimiento para obtener dashboard de ingeniero
CREATE PROCEDURE sp_dashboard_ingeniero(
    IN p_id_empresa INT
)
BEGIN
    -- Total clientes
    SELECT COUNT(*) AS total_clientes FROM clientes WHERE id_empresa = p_id_empresa AND deleted_at IS NULL;
    
    -- Total lotes
    SELECT COUNT(*) AS total_lotes FROM lotes WHERE id_empresa = p_id_empresa AND deleted_at IS NULL;
    
    -- Etapas en proceso
    SELECT COUNT(*) AS etapas_proceso FROM etapas_cultivo 
    ...
    -- Próximas cosechas (15 días)
    SELECT ec.*, l.nombre AS lote_nombre, 
...
-- 9. Procedimiento para autenticar usuario
...
BEGIN
    ...
    ELSE
        SELECT NULL AS id, 'Usuario no encontrado' AS mensaje;
    END IF;
END$$

-- 10. Procedimiento para obtener historial de aplicaciones por etapa
CREATE PROCEDURE sp_historial_aplicaciones(
    IN p_id_etapa INT,
    IN p_id_empresa INT
)
BEGIN
    SELECT a.*, p.nombre AS producto_nombre, p.tipo AS producto_tipo
    FROM aplicaciones a
...
('Agroquímico Salazar', '0912345678001', 'Av. Principal 123', '042-123456', 'salazar@agro.com'),
('Agroquímico Basurto', '0987654321001', 'Calle Comercio 456', '042-654321', 'basurto@agro.com');

-- Usuario administrador general (password: admin123)
INSERT INTO usuarios (id_empresa, username, password, nombre_completo, email, rol) VALUES
(NULL, 'admin', '$2y$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'Administrador General', 'admin@agrimanage.com', 'admin_general');

-- Usuarios para empresas (password: 12345678)
CALL sp_crear_usuario(1, 'ing_salazar', '$2y$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'Ing. María García', 'maria@salazar.com', 'ingeniero');
CALL sp_crear_usuario(2, 'ing_basurto', '$2y$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'Ing. Juan Pérez', 'juan@basurto.com', 'ingeniero');